...
# zmy update 20200210
installPath="/opt/DolphinScheduler/ds-backend-run"
...
# zmy update add $sshport for sshport is updated 20200210
...
#echo "sh ${workDir}/script/stop-all-local.sh"
#sh ${workDir}/script/stop-all-local.sh
...
# 5,scp resources
# zmy update if scp port is updated,please update scp-hosts.sh
echo "5,scp resources"
#sh ${workDir}/script/scp-hosts.sh
sh ${workDir}/script/cp-local.sh
if [ $? -eq 0 ]
then
...
# 6,startup
#echo "6,startup"
#sh ${workDir}/script/start-all-local.sh
